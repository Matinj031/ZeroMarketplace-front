<template>
  <v-form :readonly="loading" @submit.prevent="submit" ref="addPurchaseInvoiceForm" validate-on="submit lazy">
    <!--  Settlement Dialog  -->
    <SettlementDialog v-model="settlementDialog" :_id="settlementId" type="purchase-invoice"
      @exit="closeSettlementDialog" ref="settlementDialogRef" />
    ...
    <v-row>
      ...
      <v-col class="mt-md-0" cols="12" md="4">
        <UserInput v-model="form._supplier" label="تامین کننده" class="mt-md-3 mb-5" :rules="[rules.required]"
          :insert-dialog-icon="true" :readonly="loading" />
      </v-col>
      ...
      <v-col class="mt-n8 mt-md-3" cols="12" md="4">
        <WarehouseInput class="" label="وارد می‌شود به انبار" :rules="[rules.requiredSelect]" v-model="form._warehouse">
        </WarehouseInput>
      </v-col>

      <!--   Description   -->
      <v-col class="mt-n2 mt-md-n8" cols="12" md="12">
        <v-text-field class="" v-model="form.description" :readonly="loading" label="توضیحات" density="compact"
          variant="outlined">
        </v-text-field>
      </v-col>
    </v-row>
    ...
    <v-row class="border rounded-lg mx-5 mt-3 mb-2 pt-1 pb-2" v-for="(product, index) in form.products">
      ...
      <v-col class="pa-1 mt-2" cols="12" md="1">
        <v-text-field class="" v-model="product.count" @input="calculateProductPrices(index)" label="تعداد"
          type="number" :readonly="loading" :rules="[rules.required]" density="compact" variant="outlined" hide-details>
        </v-text-field>
      </v-col>

      <!--   Purchase Price    -->
      <v-col class="pa-1 mt-2" cols="12" md="2">
        <PriceInput class="" v-model="product.price.purchase" @update:modelValue="calculateProductPrices(index)"
          label="فی خرید" :readonly="loading" :rules="[rules.required]" />
      </v-col>

      <!--   Consumer Price    -->
      <v-col class="pa-1 mt-2" cols="12" md="2">
        <PriceInput v-model="product.price.consumer" :label="'فی مصرف' +
          (product.profitPercent ? '(%' + product.profitPercent + ')' : '')
          " :readonly="loading" :rules="[rules.required]" />
      </v-col>

      <!--   Store Price    -->
      <v-col class="pa-1 mt-2" cols="12" md="2">
        <PriceInput class="" v-model="product.price.store" :label="'فی فروشگاه' +
          (product.profitPercent ? '(%' + product.profitPercent + ')' : '')
          " :readonly="loading" :rules="[rules.required]" />
      </v-col>
