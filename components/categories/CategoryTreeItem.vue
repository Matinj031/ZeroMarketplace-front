<template>
  <div class="category-tree-item">
    <div class="category-item bg-bglight rounded-lg pa-4 mb-2 d-flex align-center">
      <!-- Expand/Collapse Button (Right side in RTL) -->
      ...
      <div v-else class="ms-2" style="width: 40px"></div>
      ...
      <div class="d-flex ga-2 me-3">
        ...
        <RouterLink to="" @click.stop="$emit('toggle-status', item)">
          <v-avatar :color="item.status === 1 ? 'lightwarning' : 'lightinfo'" size="32">
            <Icon :icon="item.status === 1 ? 'solar:eye-closed-linear' : 'solar:eye-linear'"
              :class="item.status === 1 ? 'text-warning' : 'text-info'" height="18" />
          </v-avatar>
        ...
        <!-- Delete -->
        <RouterLink to="" @click.stop="$emit('delete', item._id)">
    ...
    <!-- Children (Recursive) -->
    <div v-if="expanded && item.children && item.children.length" class="children-container me-8">
