    ...
    <v-card class="spike-card" elevation="10">
        <v-card-item>
            ...
            <div class="d-flex justify-space-between align-center mb-6">
                <h5 class="text-20">جزئیات فاکتور خرید</h5>
            ...
            <div v-if="!loading && invoice" class="bg-bglight pa-6 rounded-md">
                ...
                <v-row class="mb-4">
                    ...
                    <v-col cols="12" md="6">
                        <div class="mb-4">
                            <v-label class="font-weight-semibold pb-2">تامین کننده</v-label>
                            <div class="text-16">{{ invoice._supplier?.fullName || "-" }}</div>
                        </div>
                    </v-col>
                    <v-col cols="12" md="6">
                        <div class="mb-4">
                            <v-label class="font-weight-semibold pb-2">انبار</v-label>
                            <div class="text-16">{{ invoice._warehouse?.title || "-" }}</div>
                        </div>
                    </v-col>
                    <v-col cols="12" md="6">
                        <div class="mb-4">
            ...
            <div v-if="!loading && invoice" class="d-flex align-center justify-end ga-3 mt-6">
                <v-btn color="primary" variant="flat" rounded="pill" :to="`/purchase-invoices/edit/${invoice._id}`">
...
const loadInvoice = async () => {
    loading.value = true;
    try {
        const data = await useApiService.get(`purchase-invoices/${route.params.id}`);
        if (data) {
            invoice.value = data;
...
const confirmDelete = async () => {
    try {
        await useApiService.remove(`purchase-invoices/${route.params.id}`);
