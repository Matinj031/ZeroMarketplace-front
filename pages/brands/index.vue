<template>
    ...
    <v-card-item>
        ...
        <v-row class="d-flex flex-nowrap">
          <v-col cols="12" md="4" sm="6">
            <div :class="[
              ...
              <div>
                <h6 class="text-h6">همه برندها</h6>
                <p class="text-14 lh-normal">{{ totalCount }} برند</p>
              </div>
            </div>
          </v-col>
          <v-col cols="12" md="4" sm="6">
            <div :class="[
              ...
              <div>
                ...
                <p class="text-14 lh-normal">{{ activeCount }} برند</p>
              </div>
            </div>
          </v-col>
          <v-col cols="12" md="4" sm="6">
            <div :class="[
              ...
              <div>
                ...
                <p class="text-14 lh-normal">{{ inactiveCount }} برند</p>
              </div>
            </div>
          </v-col>
        </v-row>
      ...
      <div class="d-sm-flex justify-space-between align-center my-4">
        <v-sheet width="255" class="mb-lg-0 mb-4">
          <v-text-field v-model="searchValue" label="جستجوی برند" variant="outlined" hide-details class="w-100"
            density="compact">
        ...
        <v-btn class="spike-btn" color="primary" rounded="pill" flat to="/brands/create">
          برند جدید
        </v-btn>
      </div>
        ...
        <template v-slot:default>
          ...
          <tbody>
            <tr v-for="item in filteredList" :key="item._id" class="month-item">
              <td class="text-subtitle-1 font-weight-semibold">{{ item.title }}</td>
              <td>
              ...
              <td>
                <div class="d-flex ga-2 align-center justify-center">
                  <RouterLink :to="`/brands/edit/${item._id}`">
                  ...
                  <RouterLink to="" @click.prevent="handleToggleStatus(item)">
                  ...
                  <RouterLink to="" @click.prevent="handleDeleteBrand(item._id)">
  ...
  <v-dialog v-model="showConfirmation" max-width="500px">
    <v-card>
      <v-card-title class="pa-4 bg-primary">حذف برند</v-card-title>
      <v-card-text class="pt-4">
        <h5 class="text-16">آیا از حذف این برند اطمینان دارید؟</h5>
      </v-card-text>
      <v-card-actions>
...
const getBrands = async () => {
  loading.value = true;
  try {
    const data = await useApiService.get('brands?' + filter());
...
const handleToggleStatus = async (item) => {
  ...
  try {
    await useApiService.patch(`brands/${item._id}/status`, { status: newStatus });
...
const confirmDelete = async () => {
  if (brandIdToDelete.value) {
    try {
      await useApiService.remove(`brands/${brandIdToDelete.value}`);
      $notify('برند با موفقیت حذف شد', 'success');
      await getBrands();
    } catch (error) {
      $notify('مشکلی در حذف برند پیش آمد', 'error');
    } finally {
